...
namespace CarLogic
{
	public class SyncMoveSender
	{
		public float AsyncInterval = 0.15f;
		...
		public SyncMoveSender(CarState carState)
		{
		...
		public void SendTransform()
		{
			...
			MovementData movementData = new MovementData(carState, myIndex++, num, NetDelay);
			if (carState.ApplyingSpecialType == SpecialType.Translate)
			{
				Vector3 vector = (movementData.Position - lastPosition) / num;
				movementData.Velocity = carState.transform.forward * vector.magnitude;
			}
			movementData.CarModelRotation = carState.view.CarBody.localRotation;
			lastPosition = movementData.Position;
			lastSendTime = Time.time;
		...
		public void UpdateNetDelay(MovementData data)
		{
