...
public class UniGifImage : MonoBehaviour
{
    ...
    public IEnumerator SetGifData(TextAsset asset, Action callback, bool autoPlay = true)
    {
        // Get GIF textures
        return UniGif.GetTextureListCoroutine(asset.bytes, Callback, m_filterMode, m_wrapMode, m_outputDebugLog);

        void Callback(List<UniGif.GifTexture> spriteList, int loopCount, int width, int height)
        {
            if (spriteList != null)
            {
                m_gifTextureList = spriteList;
