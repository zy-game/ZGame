    ...
    public class UIBindGeneric
    {
        ...
        private void GenericInitializedCode(UIBindData variable, UIBindRulerItem rule)
        {
            if (rule.TypeName.Equals("UIBind"))
            {
                ...
                AddInit($"\t\t\ttemp_{bindData.NameSpace} = new Templete_{bindData.NameSpace}();");
                AddInit($"\t\t\ttemp_{bindData.NameSpace}.gameObject = this.go_{variable.name};");
        ...
        public string GetOverloadCode()
        {
            ...
            UIBindRulerConfig.instance.nameSpaces.ForEach(x => sb.AppendLine("using " + x + ";"));
            ...
            if (temp.IsNullOrEmpty() is false && temp.Contains("Resources"))
            {
