...
{
    [UIGenerator(typeof(UnityEngine.UI.InputField))]
    public class InputGenerator : IUIComponentGenerator
    {
        public void Dispose()
        {
        }
        ...
        {
            writer.SetField($"public {typeof(UnityEngine.UI.InputField).FullName} ipt_{gameObject.name} {{ get; private set; }}");
            writer.SetInit($"ipt_{gameObject.name} = this.go_{gameObject.name}.GetComponent<{typeof(UnityEngine.UI.InputField).FullName}>();");
