...
namespace ZGame.Editor
{
    public class SelectorPropertyDrawer : OdinAttributeDrawer<SelectorAttribute>
    {
        ...
        {
            if (EditorGUILayout.DropdownButton(label, FocusType.Passive, EditorStyles.toolbarPopup))
            {
        ...
        private GenericSelector<object> CreateSelector()
        {
            ...
            var colltion = source.Select(x => new GenericSelectorItem<object>(x.Text, x.Value)).ToList();
            GenericSelector<object> selector = new GenericSelector<object>("", false, colltion);
