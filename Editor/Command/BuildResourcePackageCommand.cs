    ...
    public class BuildResourcePackageCommand
    {
        public static void Executer(params object[] args)
        {
            using (ResourcePackageCollector collector = new ResourcePackageCollector())
            {
                ...
                var manifest = BuildPipeline.BuildAssetBundles(CoreAPI.GetPlatformOutputPathDir(), list.ToArray(), BuildAssetBundleOptions.None, target);
        ...
        {
            BuildPipeline.GetCRCForAssetBundle(new DirectoryInfo(CoreAPI.GetPlatformOutputPathDir()).Name, out uint crc);
            ...
            foreach (var VARIABLE in builds)
            {
                ...
                for (int i = 0; i < VARIABLE.builds.Length; i++)
                {
                    ...
                    BuildPipeline.GetCRCForAssetBundle(CoreAPI.GetPlatformOutputPath(VARIABLE.builds[i].assetBundleName), out crc);
                ...
                if (VARIABLE.seting.title == ResConfig.instance.defaultPackageName)
                {
                    ...
                    {
                        string librayPath = CoreAPI.GetPlatformOutputPath(libraryAssets[i]);
        ...
        private static void OnUploadBuilderResourcePackage(ResourcePackageListManifest manifest, int allCount, ref int successCount)
        {
            ...
            {
                successCount++;
                string scrPath = CoreAPI.GetPlatformOutputPath(bundle.name);
            ...
            UploadResourcePackageCommand.Executer(ResConfig.instance.current, CoreAPI.GetPlatformOutputPath(manifest.name + ".ini"));
        }
    }
}