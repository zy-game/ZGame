...
{
    [SubPageSetting("详细", typeof(HelpmeWindow), true)]
    public class HelpmeReaderWindow : SubPage
    {
        ...
        private void OnDrawingChetList()
        {
            ...
            if (GUILayout.Button("发表", GUILayout.Width(50)))
            {
                ...
                {
                    user = HelpmeConfig.instance.userName,
        ...
        private void OnDrawingChatInfo(ChatData VARIABLE, float width)
        {
            ...
            {
                if (VARIABLE.like.Contains(HelpmeConfig.instance.userName))
                {
                    VARIABLE.like.Remove(HelpmeConfig.instance.userName);
                }
                else
                {
                    VARIABLE.like.Add(HelpmeConfig.instance.userName);
                }

                VARIABLE.unlike.Remove(HelpmeConfig.instance.userName);
            }
            ...
            {
                if (VARIABLE.unlike.Contains(HelpmeConfig.instance.userName))
                {
                    VARIABLE.unlike.Remove(HelpmeConfig.instance.userName);
                }
                else
                {
                    VARIABLE.unlike.Add(HelpmeConfig.instance.userName);
                }
