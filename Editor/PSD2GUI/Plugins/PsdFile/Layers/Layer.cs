    ...
    public class Layer : ICloneable
    {
        ...
        public Layer(BinaryReverseReader reader, PsdFile psdFile)
        {
            ...
            foreach (AdjustmentLayerInfo adjustmentLayerInfo in AdjustmentInfo)
            {
                ...
                {
                    uint length = adjustmentLayerInfo.DataReader.ReadUInt32();
                    if (length > 0)
                    {
                        byte[] bytes = adjustmentLayerInfo.DataReader.ReadBytes((int)length);
                        uint type = adjustmentLayerInfo.DataReader.ReadUInt32();
                        Folder = (1 == type || 2 == type);
                        FolderEnd = type == 3;
                        //Name += " Folder " + FolderEnd;
                    }
                }
            }
        ...
        public object Clone()
        {
            ...
            layer.Name = Name;
            layer.AdjustmentInfo = new List<AdjustmentLayerInfo>();
            layer.AdjustmentInfo.AddRange(AdjustmentInfo);
            layer.Channels = new List<Channel>();
            layer.Channels.AddRange(Channels);
            ...
            layer.HasEffects = HasEffects;
            layer.IsTextLayer = IsTextLayer;
            layer.Justification = Justification;
            layer.MaskData = MaskData;
            layer.Opacity = Opacity;
            layer.PsdFile = PsdFile;
            layer.Rect = Rect;
            layer.SortedChannels = new SortedList<short, Channel>();
            foreach (var item in SortedChannels)
            {
                layer.SortedChannels.Add(item.Key, item.Value);
            }
            layer.Text = Text;
            layer.WarpStyle = WarpStyle;
            return layer;
        }
