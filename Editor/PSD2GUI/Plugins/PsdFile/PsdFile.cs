    ...
    public class PsdFile
    {
        ...
        public void SortLayer()
        {
            for (int i = Layers.Count - 1; i >= 0; i--)
            {
                if (!Layers[i].Visible)
                {
                    if (Layers[i].Name.ToLower().StartsWith("font")
                        || Layers[i].Name.ToLower().StartsWith("image")
                        || Layers[i].Name.ToLower().StartsWith("toolbar")
                        || Layers[i].Name.ToLower().StartsWith("scroll")
                        || Layers[i].Name.ToLower().StartsWith("button")
                        || Layers[i].Name.ToLower().StartsWith("input")
                        || Layers[i].Name.ToLower().StartsWith("dorp")
                        || Layers[i].Name.ToLower().StartsWith("bar")
                        || Layers[i].Name.ToLower().StartsWith("content"))
                    {
                        continue;
                    }
            ...
            List<Layer> deletes = new List<Layer>();
            for (int i = Layers.Count - 1; i >= 0;)
            {
                if (!Layers[i].Folder)
                {
                    i--;
                ...
                List<Layer> group = GetLayerGroup(i, Layers[i]);
                deletes.AddRange(group);
                i -= group.Count + 1;
            }
            for (int i = 0; i < deletes.Count; i++)
            {
                Layers.Remove(deletes[i]);
            }
            for (int i = Layers.Count - 1; i >= 0; i--)
            {
                if (Layers[i].FolderEnd || Layers[i].Name == "</layergroup>")
                {
                    Layers.Remove(Layers[i]);
                }
            }
        ...
        private List<Layer> GetLayerGroup(int index, Layer parent)
        {
            ...
            for (int i = index - 1; i >= 0;)
            {
                ...
                if (temp.Folder)
                {
                    List<Layer> group = GetLayerGroup(i, temp);
                    result.AddRange(group);
                    i -= group.Count + 1;
                    continue;
                }
                if (temp.FolderEnd || temp.Name == "</layergroup>")
                {
