...
namespace ZGame
{
    public class MathEx
    {
        public static Vector3[] GetSphereSharingPoints(int stepCount, float radius)
        {
            ...
            for (float alpha = -(float)Math.PI + 0.01f; alpha <= (float)Math.PI; alpha += step)
            {
                for (float beta = 0; beta < Math.PI * 2; beta += step)
                {
                    float x = radius * (float)(Math.Sin(alpha) * Math.Sin(beta));
                    float y = radius * (float)(Math.Cos(alpha) * Math.Sin(beta));
                    float z = radius * (float)Math.Cos(beta);
                    g_points.Add(new Vector3(x, y, z));
                }
            }
            //需要优化最后一圈和第一圈，每个点都聚集在一个位置，并且需要加入一个角度的参数，用来控制每个点的偏移

            return g_points.ToArray();
        }
