...
namespace TrueSync
{
    ...
    public class Physics2DSimulator : IPhysicsManager
    {
        ...
        public FP LockedTimeStep { get; set; }
        ...
        public void AddBody(ICollider iCollider)
        {
            ...
            gameObjectMap[tsCollider.Body] = tsCollider.gameObject;
            ...
            if (tsCollider.gameObject.transform.parent != null && tsCollider.gameObject.transform.parent.GetComponentInParent<TSCollider2D>() != null)
            {
                ...
                Physics2D.Body childBody = (Physics2D.Body)tsCollider.Body;

                childBody.bodyConstraints.Add(new ConstraintHierarchy2D(((Physics2D.Body)parentCollider.Body), (Physics2D.Body)tsCollider.Body, (tsCollider.GetComponent<TSTransform2D>().position + tsCollider.ScaledCenter) - (parentCollider.GetComponent<TSTransform2D>().position + parentCollider.ScaledCenter)));
            }
        ...
        {
            world.BodyRemoved += delegate(TrueSync.Physics2D.Body body) { OnRemoveBody(body); };
        }
