        ...
        class OptionsConverter : JsonConverter
        {
            ...
            public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
            {
                ...
                if (objectType.IsArray is false && typeof(IList).IsAssignableFrom(objectType) is false)
                {
                    ...
                    Type type = AppDomain.CurrentDomain.GetTypeForThat(opData.type);
                    if (type is null)
                    {
                        return default;
                    }
                ...
                {
                    Type target = AppDomain.CurrentDomain.GetTypeForThat(listOpData[i].type);
                    if (target is null)
                    {
                        Debug.LogError("没找到类型：" + listOpData[i].type);
                        continue;
                    }
